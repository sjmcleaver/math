...
ℍ' x B b y p = transport (B' (type-of x) x B) (𝕁 (type-of x) (λ x y p → (x , refl x) ＝ (y , p)) (λ u → refl ((u , refl u))) x y p) b where
...
dnR-EM : (X : 𝓤 ̇ ) → (R : 𝓥 ̇ ) → is-subsingleton X → ((X + (X → R)) → R) → R
dnR-EM X R s f = f (inr (λ x → f (inl x)))
