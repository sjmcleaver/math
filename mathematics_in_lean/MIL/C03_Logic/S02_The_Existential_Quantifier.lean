...
example (divab : a ∣ b) (divbc : b ∣ c) : a ∣ c := by
  rcases divab with ⟨d, beq, refl⟩
  rcases divbc with ⟨e, ceq, refl⟩
  exact dvd_trans (dvd_mul_right a d) (dvd_mul_right (a * d) e)

example (divab : a ∣ b) (divac : a ∣ c) : a ∣ b + c := by
  rcases divab with ⟨x, beq, refl⟩
  rcases divac with ⟨y, ceq, refl⟩
  exact (mul_add a x y) ▸ (dvd_mul_right a _)
